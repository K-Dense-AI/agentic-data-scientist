    ...
    try:
        ...
        if response and response.content and response.content.parts:
            ...
            if len(parts) > 0 and parts[0].text:
                ...
                logger.info(f"[Compression] ✓ LLM generated summary ({len(summary)} chars, ~{len(summary) // 4} tokens)")
        ...
        logger.warning(f"[Compression] LLM returned empty response")
...
async def compress_events_manually(
    ...
    if end_idx <= 0:
        logger.warning(f"[ManualCompression] Not enough events to compress")
        return
    ...
    if start_idx >= end_idx:
        logger.debug(f"[ManualCompression] No new events to compress")
        return
